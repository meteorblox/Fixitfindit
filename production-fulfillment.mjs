...
export function createProductionFulfillment({path,orders,catalog,cj,enabled=false}) {
 ...
 return {stage,summary,close:()=>db.close(),sync:async id=>(await sync(id)).summary,
  async submit(id){allowed();verified(id);const j=get(id);if(!j)throw new Error('Stage verified order first');if(j.state!=='ready')return summary(id);await checkCosts(id);if(!claim(id,'ready','creating'))return summary(id);try{const cjId=await cj.create(JSON.parse(j.payload));db.prepare("UPDATE production_jobs SET cj_id=?,state='created' WHERE id=? AND state='creating'").run(cjId,id);}catch{update(id,'creating','creation_outcome_unknown');throw new Error('Creation outcome unknown; sync without resubmitting');}return (await sync(id)).summary;},
  async pay(id,approvedMaximumCents){allowed();if(!Number.isSafeInteger(approvedMaximumCents)||approvedMaximumCents<1)throw new Error('Explicit supplier payment limit required');let current=await sync(id),j=get(id);if(['paid','shipped','delivered'].includes(j.state))return summary(id);if(!['created','confirmed'].includes(j.state))throw new Error('Reconcile before payment');
   if(j.state==='created'){if(!j.cj_id)throw new Error('Shipment identity requires reconciliation');if(!claim(id,'created','confirming'))return summary(id);try{await cj.confirm(j.cj_id);}catch{update(id,'confirming','confirmation_outcome_unknown');throw new Error('Confirmation uncertain; sync first');}current=await sync(id);j=get(id);}
   ...
   if(!j.cj_id||!claim(id,'confirmed','paying'))throw new Error('Payment cannot be claimed');try{await cj.pay(j.cj_id);}catch{update(id,'paying','payment_outcome_unknown');throw new Error('Payment uncertain; sync without paying again');}return (await sync(id)).summary;
  }
 };
}
