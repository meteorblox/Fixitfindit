...
export function createProductionFulfillment({path,orders,catalog,cj,enabled=false}) {
 ...
 const columns=new Set(db.prepare('PRAGMA table_info(production_jobs)').all().map(c=>c.name));
 for(const [name,type] of [['address_review_required','INTEGER NOT NULL DEFAULT 0'],['address_review_hash','TEXT'],['address_reviewed_at','TEXT']])if(!columns.has(name))db.exec('ALTER TABLE production_jobs ADD COLUMN '+name+' '+type);
 const get=id=>db.prepare('SELECT * FROM production_jobs WHERE id=?').get(id);
 const summary=id=>{const j=get(id);return j?{orderId:id,updatedAt:j.updated,supplierReference:j.custom_id,cjOrderId:j.detail_id||j.cj_id,state:j.state,trackingNumber:j.tracking,trackingProvider:j.provider,lastError:j.error,addressReviewRequired:Boolean(j.address_review_required),addressReviewedAt:j.address_reviewed_at}:null;};
 ...
 function checkIdentity(job,d){const p=JSON.parse(job.payload);if(d?.isSandbox!==0||typeof d.orderId!=='string'||!d.orderId||(job.detail_id&&job.detail_id!==d.orderId)||![d.orderNum,d.platformOrderId].includes(job.custom_id)||d.shippingCountryCode!=='US'||!Array.isArray(d.productList)||d.productList.length!==1||d.productList[0].vid!==p.products[0].vid||d.productList[0].quantity!==1)throw new Error('Supplier order identity mismatch');
  const missing=[];
  for(const k of ['shippingZip','shippingProvince','shippingCity','shippingCustomerName','shippingAddress','shippingAddress2','logisticName','fromCountryCode']){
   if(['shippingZip','shippingAddress2'].includes(k)&&d[k]==null){missing.push(k);continue;}
   if(norm(d[k])!==norm(p[k]))throw new Error('Supplier recipient or shipping method mismatch');
  }
  return missing;
 }
 function reviewHash(job,d){return createHash('sha256').update(JSON.stringify([job.payload,d.orderId,...['shippingCountryCode','shippingProvince','shippingCity','shippingCustomerName','shippingAddress','shippingZip','shippingAddress2','logisticName','fromCountryCode'].map(k=>[k,d[k]==null?null:norm(d[k])])])).digest('hex');}
 function addressClear(id){if(get(id)?.address_review_required)throw Error('Manual address review required before payment');}

 async function sync(id){allowed();verified(id);const j=get(id);if(!j||j.state==='ready')throw new Error('Submit order first');try{const d=await cj.detail(j.detail_id||j.cj_id||j.custom_id);const missing=checkIdentity(j,d);const needsReview=missing.length>0&&j.address_review_hash!==reviewHash(j,d);db.prepare('UPDATE production_jobs SET address_review_required=? WHERE id=?').run(needsReview?1:0,id);const states={CREATED:'created',IN_CART:'created',UNPAID:'confirmed',UNSHIPPED:'paid',PENDING:'paid',PROCESSING:'paid',SHIPPED:'shipped',DELIVERED:'delivered',CANCELLED:'cancelled',CLOSED:'cancelled'},rank={creating:0,created:1,confirming:2,confirmed:3,paying:4,paid:5,shipped:6,delivered:7,cancelled:8};const state=states[d.orderStatus];if(!state)throw new Error('Unknown supplier state');const advance=rank[state]>=rank[j.state];
  db.prepare('UPDATE production_jobs SET detail_id=?,state=?,tracking=?,provider=?,error=?,updated=? WHERE id=? AND state=?').run(d.orderId,advance?state:j.state,advance&&typeof d.trackNumber==='string'?d.trackNumber:j.tracking,advance&&typeof d.trackingProvider==='string'?d.trackingProvider:j.provider,needsReview?'manual_address_review_required':advance?null:j.error,new Date().toISOString(),id,j.state);return {summary:summary(id),detail:d};
 }catch{db.prepare('UPDATE production_jobs SET address_review_required=1,address_review_hash=NULL,address_reviewed_at=NULL WHERE id=?').run(id);db.prepare('UPDATE production_jobs SET error=? WHERE id=? AND state=?').run('reconciliation_required',id,j.state);throw new Error('Reconcile supplier state before further mutations');}}
 ...
 address(id){verified(id);const j=get(id);if(!j)throw Error('No staged order');const p=JSON.parse(j.payload);return {orderId:id,cjOrderId:j.detail_id||j.cj_id,recipient:p.shippingCustomerName,line1:p.shippingAddress,line2:p.shippingAddress2,city:p.shippingCity,state:p.shippingProvince,zip:p.shippingZip,country:p.shippingCountryCode,instruction:'Compare every field with the exact order in MyCJ. Then run verify-address with this order ID and CJ order ID. This records a manual check; it does not edit CJ or make a payment.'};},
 async verifyAddress(id,cjOrderId){allowed();verified(id);refundClear(id);await sync(id);const j=get(id);if(!j?.detail_id||cjOrderId!==j.detail_id)throw Error('Exact CJ order ID required');const d=await cj.detail(j.detail_id);checkIdentity(j,d);refundClear(id);db.prepare('UPDATE production_jobs SET address_review_hash=?,address_reviewed_at=?,address_review_required=0,error=NULL WHERE id=?').run(reviewHash(j,d),new Date().toISOString(),id);return summary(id);},
 list(){return db.prepare('SELECT id FROM production_jobs ORDER BY updated DESC LIMIT 200').all().map(row=>{const o=orders.get(row.id);return {...summary(row.id),productName:o?.product_name,retailCents:o?.retail_cents,fulfillmentHold:orders.refunds?.summary(row.id).fulfillmentHold||false};});},close:()=>db.close(),sync:async id=>(await sync(id)).summary,
