...
test('automatic catalog checkout carries trusted price through Stripe and a paid stored order',async()=>{
 ...
 const checkout=createCheckout({key:'sk_test_fixture',orders,request:async(url,options)=>{
  if(options.body){submitted=new URLSearchParams(options.body);const metadata={};for(const [k,v]of submitted)if(k.startsWith('metadata['))metadata[k.slice(9,-1)]=v;session={id:'cs_test_auto',livemode:false,url:'https://checkout.stripe.com/test',client_reference_id:owner,currency:'usd',status:'open',payment_status:'unpaid',amount_subtotal:2000,automatic_tax:{enabled:true,status:'requires_location_inputs'},metadata};}
 ...
 const quote=await run(route,'/quote','product=p1&category=cleaning&variant=v1&zip=60601&retailCents=1');assert.equal(quote.status,200);assert.match(quote.html,/\$20.00/);const id=quote.html.match(/name="quote" value="([^"]+)"/)[1];
 const started=await run(route,'/start','quote='+id+'&retailCents=1&product=other');assert.equal(started.status,303);assert.equal(submitted.get('line_items[0][price_data][unit_amount]'),'2000');assert.equal(submitted.get('metadata[product_id]'),'p1');assert.equal(submitted.get('metadata[variant_id]'),'v1');
 session={...session,status:'complete',payment_status:'paid',amount_total:2160,total_details:{amount_shipping:0,amount_tax:160,amount_discount:0},automatic_tax:{enabled:true,status:'complete'},collected_information:{shipping_details:{address:{country:'US',postal_code:'60601'}}}};
 orders.recordSession(session,'evt_auto','checkout.session.completed');const paid=await checkout.verifyProduct(session.id,owner);assert.equal(paid.paid,true);assert.equal(paid.retailCents,2000);assert.equal(orders.get(session.metadata.order_id).status,'paid_sandbox');
