...
## Final partner checkout verification — 2026-09-17 UTC
- Deployed browser flow: Home Helper referral → touchless faucet → fixed synthetic Chicago address → Stripe test card.
- Order e85f9fb8-a47a-432a-b1e0-4a748a25ca7e: product $127.00, included shipping $0, sandbox tax $13.34, total $140.34.
- Signed payment notification recorded automatically; demo-home-helper commission basis 12700 cents, gross 635 cents; CJ sandbox ready, no supplier order submitted.
- Full Stripe sandbox refund 14034 cents succeeded; automatic refund notification reduced net commission to zero, reversed 635 cents, fulfillmentHold=true, payableCents=0.
- Deployment currently uses Stripe test credentials and CJ sandbox mode. No real funds moved.
- Launch remains blocked on live checkout implementation/configuration, authenticated live webhook address resolution and refund tracking, Stripe live account/tax readiness, and real CJ fulfillment/payment operating setup. Do not enable live flags based only on this sandbox pass.

## Live checkout preparation — 2026-09-17
Separate /checkout/live route added, disabled by default. Requires persistent order storage, LIVE_ORDER_INTAKE=enabled, LIVE_CHECKOUT=enabled, STRIPE_LIVE_SECRET_KEY (sk_live_), STRIPE_LIVE_WEBHOOK_SECRET. Keep existing sandbox credentials unchanged.
Live signed notifications resolve the fixed shipping address from Stripe, stage a production review job, and reconcile refunds. Refund GET reconciliation remains available with purchases disabled. CJ submission/payment remains an explicit private operator action; the web process never purchases supplier stock automatically.
Before enabling LIVE_CHECKOUT: configure live Stripe endpoint /webhooks/stripe-live for checkout.session.completed, checkout.session.async_payment_succeeded, checkout.session.async_payment_failed, checkout.session.expired, refund.created, refund.updated, refund.failed; verify live account, webhook delivery, actual tax registrations and CJ fulfillment operation. Storefront preview labels/links and partner dashboard launch presentation still need switching as part of activation. No live key or webhook secret was installed by this change, and no live order or charge was created.
