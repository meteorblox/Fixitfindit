...
export function createOrderStore(path) {
  ...
  function prepare(reference,item) {
    ...
    db.prepare('INSERT OR IGNORE INTO orders(id,checkout_key,owner_hash,product_id,variant_id,product_name,retail_cents,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(randomUUID(),key,hash(reference),item.productId,item.variantId,item.name,item.retailCents,now,now);
    const row=db.prepare('SELECT * FROM orders WHERE checkout_key=?').get(key);
    if(row.retail_cents!==item.retailCents || row.product_id!==item.productId) throw new Error('Price changed. Start a fresh checkout.');
    return row;
  }
  function recordSession(s,eventId=null,eventType='') {
    ...
    try {
      ...
      if(!row || row.owner_hash!==hash(s.client_reference_id||'') || row.product_id!==m.product_id || row.variant_id!==m.variant_id || m.quantity!=='1' || String(row.retail_cents)!==m.retail_cents || s.amount_total!==row.retail_cents || s.currency!=='usd' || (row.session_id && row.session_id!==s.id)) throw new Error('Order snapshot mismatch.');
  ...
  return {prepare,recordSession,get:id=>db.prepare('SELECT * FROM orders WHERE id=?').get(id),close:()=>db.close()};
}
