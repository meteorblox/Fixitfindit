...
test('automatic prices double landed cost and satisfy the floor even on cheap products',()=>{
 assert.equal(automaticRetail(500,500),2000);
 for(const cost of [0,1,99,200,500,1000,10001,99999]) {
  const price=automaticRetail(cost,333);
  assert.ok(price>=2*(cost+333));
  assert.ok(contribution(price,cost,333)>=500);
...
test('quotes reject unknown fees, stock, foreign ZIPs and excluded methods',()=>{
 assert.equal(deliveredPrice(input).retailCents,2000);
...
test('catalog quote renders delivered retail only, escapes names, and changes with shipping',()=>{
 ...
 assert.ok(html.includes('$20.00'));assert.ok(!html.includes('$5.00'));
 const higher=productOptions({...input,shipping:[{...method,totalCents:1000}]});
 assert.ok(higher.includes('$30.00'));
 const unknown=productOptions({...input,shipping:undefined});assert.ok(!unknown.includes('$20.00'));assert.ok(unknown.includes('Get delivered price'));
