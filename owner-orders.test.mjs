...
test('only authenticated owner sees paid orders, holds, unprepared orders and escaped supplier data',async()=>{
 ...
 try{assert.match((await run()).body,/Owner sign-in/);assert.equal(reads,0);const initial=await run(),csrfCookie=initial.headers['Set-Cookie'].split(';')[0],csrf=csrfCookie.split('=')[1];const code=access.issue(owner);assert.equal((await run({method:'POST',path:'/owner/orders/login',body:'code='+code,origin:'https://evil.example'})).status,403);const login=await run({method:'POST',path:'/owner/orders/login',cookie:csrfCookie,body:'code='+code+'&csrf='+csrf,origin:'null'});assert.equal(login.status,303);const cookie=login.headers['Set-Cookie'].split(';')[0]+'; '+csrfCookie;assert.match(login.headers['Set-Cookie'],/HttpOnly; Secure; SameSite=Strict/);const r=await run({cookie});assert.match(r.body,/order-private/);assert.match(r.body,/Refund hold/);assert.match(r.body,/Not submitted/);assert.ok(!r.body.includes('<script>'));assert.equal(r.headers['Cache-Control'],'private, no-store');assert.equal((await run({cookie,method:'POST',path:'/owner/orders/pay',body:'csrf='+csrf})).status,404);await run({cookie,method:'POST',path:'/owner/orders/logout',body:'csrf='+csrf});assert.match((await run({cookie})).body,/Owner sign-in/);}finally{access.close();}
});
