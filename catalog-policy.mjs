...
export function isReplacementPart(value) {
  ...
  return /^sheath\b/i.test(standalone) || /\b(?:replacement|spare)\s+(?:(?:brush|mop|filter)\s+)?(?:heads?|parts?|pads?|filters?|cartridges?|blades?|brushes|covers?)\b|\brefills?\b|\b(?:head|pad|filter|blade)\s+only\b/i.test(standalone);
}
