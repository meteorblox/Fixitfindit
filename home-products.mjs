...
export async function homeProducts(catalog, prefix = '') {
  ...
  for (const [slug,title] of sections) {
    ...
    rows.push(`<section class="home-product-section shell"><div class="home-product-heading"><h2>${title}</h2><a href="${categoryUrl}">View all →</a></div><div class="home-product-grid">${products.map(p=>`<a class="home-product-card" href="${categoryUrl}/product/${encodeURIComponent(p.id)}"><div class="home-product-photo"><img loading="lazy" src="${escape(p.image)}" alt="${escape(p.name)}"></div><div class="home-product-info"><h3 title="${escape(p.name)}">${escape(shortTitle(p.name))}</h3><p class="home-product-price">${retailPrice(p) || 'Temporarily unavailable'}</p>${p.storefrontVariants?`<small>Option: ${escape(p.storefrontVariants[0].name)}</small>`:''}<small>${retailPrice(p)?"Standard shipping included where available · Coming soon":"Temporarily unavailable"}</small><span>View details ↗</span></div></a>`).join('')}</div></section>`);
  }
  return `<div id="products"><div class="shell home-product-intro"><p>Explore our home and kitchen collection. Live purchases are coming soon.</p></div>${rows.length?rows.join(''):'<p class="shell home-product-empty" role="status">Products are temporarily unavailable. Please try the category links or check back shortly.</p>'}</div>`;
}
