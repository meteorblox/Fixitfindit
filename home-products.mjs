...
export async function homeProducts(catalog, prefix = '') {
  ...
  for (const [slug,title] of sections) {
    ...
    rows.push(`<section class="home-product-section shell"><div class="home-product-heading"><h2>${title}</h2><a href="${categoryUrl}">View all →</a></div><div class="home-product-grid">${products.map(p=>`<a class="home-product-card" href="${categoryUrl}/product/${encodeURIComponent(p.id)}"><div class="home-product-photo"><img loading="lazy" src="${escape(p.image)}" alt="${escape(p.name)}"></div><div class="home-product-info"><h3 title="${escape(p.name)}">${escape(shortTitle(p.name))}</h3><p class="home-product-price">${retailPrice(p) || (p.supplierPrice?`${escape(p.supplierPrice)} USD`:'Price unavailable')}</p><small>${retailPrice(p)?"Planned price · Standard shipping included where available · Coming soon":"CJ supplier price · Shipping extra"}</small><span>View details ↗</span></div></a>`).join('')}</div></section>`);
  }
