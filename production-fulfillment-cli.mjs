...
try {
 ...
 if(action!=='list'&&(!id||!['status','submit','sync','pay'].includes(action)))throw new Error('Usage: production-fulfillment-cli.mjs list or status|submit|sync ORDER_ID or pay ORDER_ID MAX_CENTS');
 ...
 console.log(JSON.stringify(await ({list:worker.list,status:worker.summary,submit:worker.submit,sync:worker.sync,pay:worker.pay}[action])(id,Number(limit))));
