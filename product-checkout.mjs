  ...
  return async (req,res,url)=>{
    ...
    try {
      if(url.pathname==='/checkout/products' && ['GET','HEAD'].includes(req.method)) {
        ...
        page(200,result.paid?`Stripe confirmed the ${usd(result.retailCents)} product test payment. The exact product and variant are recorded in Stripe. Nothing will ship.`:'Payment is not confirmed yet. Refresh to check again.');
