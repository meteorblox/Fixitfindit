...
export function productCheckoutPage({message='',enabled=false,chosen='',quotes=[],context=null,zip='',mode='sandbox' }={}) {
...
export function createProductCheckoutRoute({catalog,checkout=createCheckout(),quotes=shippingQuotes,requireAddress=true,resolvePartner=()=>null,mode='sandbox'}={}) {
  ...
  return async (req,res,url)=>{
    ...
    const page=(status,message='',enabled=false,options=[])=>{
      ...
      res.end(req.method==='HEAD'?undefined:productCheckoutPage({message,enabled,chosen:options[0]?.variantId||url.searchParams.get('variant'),quotes:options,context,zip:destinationZip,mode}));
    };
    ...
    try {
      if(live&&!checkout.enabled()){page(503,'Checkout is not open yet. Please check back shortly.');return true;}
