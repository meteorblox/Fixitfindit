...
export function createProductCheckoutRoute({catalog,checkout=createCheckout(),quotes=shippingQuotes,requireAddress=true,resolvePartner=()=>null,mode='sandbox',customerTracking=null}={}) {
  ...
  return async (req,res,url)=>{
    ...
    const page=(status,message='',enabled=false,options=[])=>{
      ...
      res.end(req.method==='HEAD'?undefined:productCheckoutPage({message,enabled,chosen:options[0]?.variantId||url.searchParams.get('variant'),quotes:options,context,zip:destinationZip,mode,trackingLink}));
    };
    ...
    try {
      if(live&&!checkout.enabled()&&url.pathname!==routePath+'/result'){page(503,'Checkout is not open yet. Please check back shortly.');return true;}
      if(url.pathname===routePath && ['GET','HEAD'].includes(req.method)) {
        res.setHeader('Set-Cookie',`${cookieName}=${reference||randomUUID()}; Path=${routePath}; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`);
      ...
      } else if([routePath+'/quote',routePath+'/start'].includes(url.pathname) && req.method==='POST') {
        ...
        if(url.pathname===routePath+'/quote') {
          ...
          if(eligible){const partner=resolvePartner(req);if(partner&&(!live||!partner.id.startsWith('demo-')))eligible.partner=partner;}
        ...
        try {
          ...
          item={...refreshed,...(item.partner?{partner:item.partner}:{}),shipping:{...refreshed.shipping,...(item.shipping.recipient?{recipient:item.shipping.recipient}:{})},quoteId:item.quoteId};
      ...
      } else if(url.pathname===routePath+'/result' && req.method==='GET') {
        if(!reference) {page(400,live?'Use your saved private tracking link, or contact fixitfindits@gmail.com for help.':'Test cookie missing. Start another product test.');return true;}
        const result=await checkout.verifyProduct(url.searchParams.get('session_id'),reference);
        if(live&&result.paid&&result.webhookReceived&&result.orderId)trackingLink=customerTracking?.link(result.orderId)||null;
        page(200,live?(result.paid?`Payment received: ${usd(result.totalCents)}. Order ${result.orderId} is saved. ${result.webhookReceived?'Payment confirmation recorded; your order is awaiting fulfillment review.':'We are waiting for final payment confirmation.'} For order questions, contact fixitfindits@gmail.com.`:'Payment is not confirmed yet. Refresh to check again.'):result.paid?`Stripe confirmed the ${usd(result.totalCents??result.retailCents)} product test payment. Product: ${usd(result.retailCents)}; shipping: ${usd(result.shippingCents??0)}; tax: ${usd(result.taxCents??0)}. ${result.orderId?'Order '+result.orderId+' is saved in FixItFindIt.':'The exact product and variant are recorded in Stripe.'} ${result.webhookReceived?'Automatic Stripe notification received and recorded.':'Automatic notification has not been recorded yet; refresh to check.'} ${result.destinationMatches===false?'The Stripe address differs from the shipping quote. A new quote would be required before fulfillment.':''} ${result.fulfillment?'CJ sandbox status: '+result.fulfillment.state+'. '+(result.fulfillment.trackingNumber?'Simulated tracking: '+result.fulfillment.trackingNumber+'. ':''):''}Nothing will ship.`:'Payment is not confirmed yet. Refresh to check again.');
      } else page(405,'This checkout action is unavailable.');
    } catch {page(503,live?'Checkout could not be started or verified. Please try again or contact fixitfindits@gmail.com.':'The product test could not be started or verified. Please start another test.');}
    return true;
  };
}
