...
export function catalogPage(shell, {store, category, data, error, product}) {
  ...
  const card = p=>`<article class="product"><a class="catalog-photo" href="${prefix}/category/${category.slug}/product/${encodeURIComponent(p.id)}">${p.image?`<img loading="lazy" src="${esc(p.image)}" alt="${esc(p.name)}">`:'<span>Photo unavailable</span>'}</a><div class="product-body"><p class="category">${esc(category.name)}</p><h3>${esc(p.name)}</h3><p>CJ supplier price: ${esc(p.supplierPrice || 'Unavailable')} ${p.supplierPrice?'USD':''}</p><p class="catalog-note">Shipping and retail price pending.</p><a href="${prefix}/category/${category.slug}/product/${encodeURIComponent(p.id)}">View product →</a></div></article>`;
  ...
  if (product) {
    content += `<a href="${prefix}/category/${category.slug}">← Back to ${esc(category.name)}</a><div class="catalog-detail">${product.image?`<img src="${esc(product.image)}" alt="${esc(product.name)}">`:''}<div><h2>${esc(product.name)}</h2><p>CJ supplier price: ${esc(product.supplierPrice || 'Unavailable')} ${product.supplierPrice?'USD':''}</p><p>US warehouse search result. Variant availability, delivered cost and delivery estimate still need verification.</p><p>${product.listings} CJ listings · This measures seller listings, not units sold.</p><p>${product.hasVideo?'CJ indicates a video is available. Playback will be added after checking access and usage rights.':'No video indicated in the catalog response.'}</p><strong>Not available to purchase yet</strong></div></div>`;
