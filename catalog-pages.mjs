...
export function catalogPage(shell, {store, category, data, error, product}) {
  ...
  const card = p=>`<article class="product"><a class="catalog-photo" href="${prefix}/category/${category.slug}/product/${encodeURIComponent(p.id)}">${p.image?`<img loading="lazy" src="${esc(p.image)}" alt="${esc(p.name)}">`:'<span>Photo unavailable</span>'}</a><div class="product-body"><p class="category">${esc(category.name)}</p><h3>${esc(p.name)}</h3><p>${price(p)}</p><p class="catalog-note">${retailPrice(p)?"Coming soon · Final delivery details pending":"Shipping and retail price pending."}</p><a href="${prefix}/category/${category.slug}/product/${encodeURIComponent(p.id)}">View product →</a></div></article>`;
