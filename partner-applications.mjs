...
export function createPartnerStore(path) {
 ...
 db.exec('CREATE TABLE IF NOT EXISTS partner_branding(partner_id TEXT PRIMARY KEY,show_name INTEGER NOT NULL DEFAULT 1)');
 const approved=id=>Boolean(db.prepare("SELECT id FROM partner_applications WHERE id=? AND status='approved'").get(id));
 return {
 saveBranding(id,name,showName){name=String(name||'').trim();if(!approved(id)||!name||name.length>80)throw Error('Enter a storefront name up to 80 characters');db.prepare("UPDATE partner_applications SET brand=? WHERE id=? AND status='approved'").run(name,id);db.prepare('INSERT INTO partner_branding VALUES(?,?) ON CONFLICT(partner_id) DO UPDATE SET show_name=excluded.show_name').run(id,showName?1:0);},
 ...
 find(slug){const r=db.prepare("SELECT id,brand,slug,COALESCE((SELECT show_name FROM partner_branding WHERE partner_id=partner_applications.id),1) show_name,(SELECT version FROM partner_logos WHERE partner_id=partner_applications.id) logo_version FROM partner_applications WHERE slug=? AND status='approved'").get(slug);return r?{id:r.id,slug:r.slug,name:r.brand,showName:Boolean(r.show_name),logoUrl:r.logo_version?'/partner-logos/'+r.slug+'?v='+r.logo_version:null,tagline:'Useful finds for your home.',accent:'#167164',demo:false}:undefined;},
 close(){db.close();}
 };
}
